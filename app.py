...
def main(user, domain, data):
    ...
    with st.form('myform'):
        ...
        for i, d in enumerate(data):
            with st.container():
                a, b = st.columns(columns_split)
                a.markdown(f'**{d["title"]}**')
                a.caption(d['content'][:300].replace('\n','. ')+'...')
                a.write('')
                a.write('')
                relevant[d['index']] = b.radio(label='Score:', key=f'C{i}', options=reverse(['Totally Irrelevant', 'Somewhat Irrelevant', 'Somewhat Relevant', 'Very Relevant']))
